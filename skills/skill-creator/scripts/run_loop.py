...
Usage:
    python run_loop.py --eval-set <eval.json> --skill-path <skill-dir> --model opencode/deepseek-v4-flash-free
"""
        ...
        if verbose:
            def print_eval_stats(label, results, elapsed):
                ...
                for r in results:
                    ...
                    print(f"  [{status}] rate={rate_str} expected={r.get('should_trigger', True)}: {r['query'][:60]}", file=sys.stderr)
...
def main():
    parser = argparse.ArgumentParser(description="Run eval + improve loop for skill descriptions")
    ...
    parser.add_argument("--report", default="auto", help="Generate HTML report at this path ('auto' for temp file, 'none' to disable)")
    parser.add_argument("--results-dir", default=None, help="Save all outputs (results.json, report.html, logs) to a timestamped subdirectory here")
