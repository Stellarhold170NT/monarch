export function buildHardBlocksSection() {
  const blocks = [
    ...
    "- Delivering final answer before collecting Igris result - **Never.**",
  ];
...
export function buildAntiPatternsSection() {
  const patterns = [
    ...
    "- **Igris**: Delivering answer without collecting Igris results",
  ];
...
export function buildToolCallFormatSection() {
...
export function buildUltraworkSection(agents = [], categories = [], skills = []) {
  const lines = [];
  ...
  if (agents.length > 0) {
    const ultraworkAgentPriority = ["explore", "librarian", "plan", "igris"];
...
export function buildAntiDuplicationSection() {
