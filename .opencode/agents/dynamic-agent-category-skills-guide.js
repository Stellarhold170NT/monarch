...
export function buildCategorySkillsDelegationGuide(categories, skills) {
  const skillsArray = Array.isArray(skills) ? skills : [];
  if (categories.length === 0 && skillsArray.length === 0) {
    return "";
  }
  ...
  const customSkills = skillsArray.filter((skill) => skill.location !== "plugin");
  const skillsSection = buildSkillsSection(skillsArray);
