...
const SECRET_PATTERNS: SecretPattern[] = [
  ...
  // OpenAI API key (negative lookahead avoids matching Anthropic keys)
  {
    entityType: EntityType.API_KEY,
    regex: /\bsk-(?:proj-)?[A-Za-z0-9]{20,}T3BlbkFJ[A-Za-z0-9]{20,}|\bsk-(?!ant-api)[A-Za-z0-9\-_]{32,}/g,
    confidence: 0.99,
    enabled: true,
  },
  // Anthropic API key
...
export class SecretsEngine {
  ...
  constructor() {
    this._patterns = SECRET_PATTERNS.map((p) => ({ ...p }));
  }
  ...
  addPattern(
    ...
    this._patterns.push({ entityType, regex, confidence, enabled: true, useGroup });
  }
