 ...
 * const clean = session.anonymize(userPrompt);
...
export class Session {
  ...
  anonymize(text: string): string {
    const result = this._sanitizer._run(text, this._vault, undefined, this.sessionId);
    return result.text;
  }

  /** Like {@link anonymize} but returns the full {@link SanitizeResult}. */
  anonymizeWithResult(text: string): SanitizeResult {
